...
A curated dataset of commercial DNA size markers / ladders used in agarose gel electrophoresis.

Each entry includes the product name, supplier, official product page URL, and the full band profile (fragment length, relative mass, and whether the band is a reference/hidden band).
...
| File | Description |
|------|-------------|
| `products.json` | Structured data (all products, nested by supplier) |
| `products.csv` | Flat table, one row per band (Excel-friendly, UTF-8 BOM) |
| `suppliers/` | One Markdown file per supplier |
...
| Geneaid | 2 | `geneaid.com` | [`suppliers/Geneaid.md`](suppliers/Geneaid.md) |
| GeneDireX | 8 | `genedirex.com` | [`suppliers/GeneDireX.md`](suppliers/GeneDireX.md) |
| GeneMate | 3 | `bioexpress.com` | [`suppliers/GeneMate.md`](suppliers/GeneMate.md) |
| Genessee Scientific | 12 | `geneseesci.com` | [`suppliers/Genessee_Scientific.md`](suppliers/Genessee_Scientific.md) |
| GenScript | 3 | `genscript.com` | [`suppliers/GenScript.md`](suppliers/GenScript.md) |
...
| MilliporeSigma | 13 | `sigmaaldrich.com` | [`suppliers/MilliporeSigma.md`](suppliers/MilliporeSigma.md) |
| New England Biolabs | 14 | `neb.com` | [`suppliers/New_England_Biolabs.md`](suppliers/New_England_Biolabs.md) |
| NIPPON GENE | 5 | `nippongene.com` | [`suppliers/NIPPON_GENE.md`](suppliers/NIPPON_GENE.md) |
| NZYTech | 8 | `nzytech.com` | [`suppliers/NZYTech.md`](suppliers/NZYTech.md) |
| Penn State DNA Ladders | 3 | `personal.psu.edu` | [`suppliers/Penn_State_DNA_Ladders.md`](suppliers/Penn_State_DNA_Ladders.md) |
...
```json
{
  "<Supplier Name>": {
    ...
    "ladders": [
      {
        "name": "<product name>",
        "preferred_units": "bp | kb",
        "url": "<product page URL>",
        "fragments": [
          {
            "length": <int, base pairs>,
            "mass": <number, relative band intensity>,
            "format": "bold | hidden | <empty>"
          }
        ]
      }
    ]
...
| Field | Meaning |
|-------|---------|
| `length` | Fragment length in base pairs (bp) |
| `mass` | Relative mass/intensity of the band — larger value = thicker/brighter band |
| `format` | Band display style: `bold` = reference band (emphasised on the gel); `hidden` = defined but not shown; empty = normal |
| `preferred_units` | Display unit the supplier recommends (`bp` or `kb`) |

## License

The band data (length/mass/format) in this dataset is factual product specification information. Product names, URLs and brand names belong to their respective owners and are referenced here for identification purposes only.
